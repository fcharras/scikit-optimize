...
class BatchCompletionCallBack(object):
    ...
    def __call__(self, out):
        self.parallel._async_buffer = out
...
%s""" % (this_report, exception.message)
    ...
    def get_last_async_result(self):
        '''
        Public function that can be used to implement callbacks on last
        computed result.
        '''
        if self._async_buffer is None:
            return []

        if hasattr(self._async_buffer, 'result'):
            return self._async_buffer.result()

        if hasattr(self._async_buffer, 'get'):
            return self._async_buffer.get()

        return self._async_buffer
